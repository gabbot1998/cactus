  ...
  (:require
   ...
   [cactus.async
    ...
    :refer [size? go <<! chan]
     ...
     [cactus.actor_macros
     :as cactus.actors
     :refer [defentity con defaction >>! guard defstate --]
     ]
...
; (defentity sw-cell-printing-old [a-length tot-rows] [a-chan b-chan west] ==> [value aligner-value]
...
;   )
  ...
  (defentity sw-cell-printing [a-length tot-rows] [a-chan b-chan west] ==> [value aligner-value]
    (defstate [nw 0 n 0 i 0 j 0])
    (defaction a-chan [a] b-chan [b] west [new-west] ==>
      (let [new-nw new-west
            new-n (cell-action @nw @n new-west a b)
...
(defentity stripe-actor [a-length] [b-chan] ==> [chan-0 chan-1 chan-2 chan-3 ]
  (defaction b-chan [bs] ==>
...
(defentity fanout-actor [] [in-chan] ==> [chan-0 chan-1 chan-2 chan-3 ]
...
(defentity align-actor [A B stripe-width] [chan-0 chan-1 chan-2 chan-3] ==> [out]
  (defstate [row 0
